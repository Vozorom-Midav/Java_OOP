    ...
    public boolean receiveOffer(String nameCompany, Vacancy companyVacancy) {
        if(companyVacancy.getVacancyType().equals(this.profession)){
            if (this.salary <= companyVacancy.getSalary()){
                ...
                System.out.printf("Специалист %s: Зарплата меня не устраивает! (компания: %s; заработная плата: %d)\n",
                        name, nameCompany, companyVacancy.getSalary());
                        return false;
            }
        }
        else {
            System.out.printf("Специалист %s: Это не моя профессия! (компания: %s; Предлагали профессию: %s; Моя профессия: %s)\n",
